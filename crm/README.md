...
### ו. ניטור שרתים וקונטיינרים (Server Monitor)
*   **ניטור חומרה ומשאבים בזמן אמת:** תצוגה גרפית של עומס המעבד (CPU), ניצול זיכרון (RAM) ונפח האחסון בשרת Hetzner.
*   **מעקב קונטיינרים דינמי:** רשימה של קונטיינרי Docker הרצים בשרת (כולל N8N, PostgreSQL ועוד) עם חיווי סטטוס בריאות צבעוני (ירוק ל-running) וצריכת משאבים ספציפית לכל קונטיינר.
*   **אבטחה וקריאה בלבד (Read-Only):** הממשק מבוסס על משיכת נתונים בלבד באמצעות סקריפט Python בחיבור SSH ופונקציות Supabase RPC. הוסרו כפתורי שליטה (הפעלה מחדש/כיבוי) למניעת השבתה לא מתוכננת של שירותים קריטיים.
